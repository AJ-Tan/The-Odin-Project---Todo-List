...
const AddTodo = (projectList) => {
  ...
  modalTitleNode.textContent = "Add Todo";
  ...
  modalInputContainerNode.appendChild(todoDeadlineFormGroup);
  ...
  todoDeadlineInput.value = format(new Date(), "yyyy-MM-dd");
  ...
  projectSubmit.textContent = "Add Todo";
  modalInputContainerNode.appendChild(projectSubmit);

  modalFormNode.addEventListener("submit", e => {
    e.preventDefault();
    const {todoTitle, todoPriority, todoDeadline, todoDescription} = Object.fromEntries(new FormData(modalFormNode));
    ...
    todoData.addTodo(projectList, todo);
    renderContentBody();
    modalNode.close();
  })
